...
export default function SingleFileAudit() {
  ...
  const handleSopUpload = async (file: File) => {
    ...
    try {
      ...
      // User can now review extracted rules before clicking Next
    } catch (err: any) {
  ...
  const handleClaimUpload = async (file: File) => {
    ...
    try {
      ...
      // User can now review extracted claim data before clicking Next
    } catch (err: any) {
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          >
            ...
            <div className="text-lg opacity-90 space-y-1">
              <p>SOP Document used for this audit: {sopFile?.name || 'N/A'}</p>
              <p>Claim Document used for this audit: {claimFile?.name || 'N/A'}</p>
              <p>Claim ID: {results.claim_id || 'N/A'}</p>
            </div>
          </div>
