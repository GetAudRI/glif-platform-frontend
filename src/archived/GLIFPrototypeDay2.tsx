...
import { testConnection, testAuditModule, listClaims, listSOPs, startAudit } from './services/api';
...
export default function GLIFPrototype() {
  ...
  async function handleStartAudit() {
    ...
    try {
      ...
      alert(`Audit started! ID: ${result.audit_id}\n${result.message}`);
    } catch (err: any) {
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">5-step audit workflow - Real file selection</p>
                  </div>
                  <button
                    onClick={() => {
                      if (!auditStarted) {
              ...
              <CardContent>
                {!auditStarted ? (
                  ...
                  <div className="space-y-6">
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="flex justify-end">
                              <button
                                ...
                                disabled={selectedClaims.length === 0 || selectedSOPs.length === 0}
                                className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium disabled:bg-gray-300 disabled:cursor-not-allowed"
                              >
                                Begin Audit →
                              </button>
                            </div>
                    ...
                    {/* Audit Started - Show Progress */}
                    {auditId && (
                      <div>
                        <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg mb-6">
                          <p className="font-semibold">✅ Audit Started!</p>
                          <p className="text-sm mt-1">
                            Audit ID: {auditId} | {selectedClaims.length} claims | {selectedSOPs.length} SOPs
                          </p>
                        </div>

                        {/* Next steps will be implemented in Day 3-5 */}
                        <div className="space-y-3">
                          {[
                            { step: 2, title: 'Data Gathering', status: 'pending' },
                            { step: 3, title: 'File Review & Evaluation', status: 'pending' },
